...
class SocialNetworkRepository extends ServiceEntityRepository
{
    ...
    */
    public function findForNav() //url not null
    {
        return $this->createQueryBuilder('s')
            ->andWhere('s.url IS NOT NULL')
            ->orderBy('s.id', 'ASC')
            ->getQuery()
            ->getResult()
        ;
    ...
    public function findForFooter() //url et fontawesome not null
    {
        return $this->createQueryBuilder('s')
            ->andWhere('s.url IS NOT NULL')
            ->andWhere('s.fontawesome IS NOT NULL')
            ->orderBy('s.id', 'ASC')
            ->getQuery()
            ->getResult()
        ;
